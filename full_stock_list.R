...
# example of how to paste spaces before and after the words in the table
#top_terms <- data.frame(table(unlist(strsplit(tolower(top_terms$top_terms), " "))))
#top_terms$Var1 <- paste(" ", top_terms$Var1, sep="")
#top_terms$Var1 <- paste(top_terms$Var1, ' ', sep='')
# not sure if it's necessary to change to character form <-- maybe do it just in case
#mct$Var1 <- as.character(mct$Var1)
# example of how to remove words from my list later on
#mct <- mct[!grepl(' to ', mct$Var1),]
# afterwards to remove all spaces, use
#mct <- mct %>% str_squish()
