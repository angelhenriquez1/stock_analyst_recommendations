...
#removing yahoo tickers with no data####
yahoo <- function(stock_sign){
   ...
   current3 <- current[13,1]
   current3 <- gsub(",","",current3)
   current3 <- suppressWarnings(as.numeric(as.character(current3)))
   
   current1 <- ifelse(is.na(current1) == TRUE, current2, current1)
   current <- ifelse(is.na(current1) == TRUE, current3, current1)
...
# identify which stocks having missing values
