...
#Bar Chart Website Recommendation####
...
data <- function(){
   ...
   stock_names <- cbind(stock_names1$`ACT Symbol`, stock_names2$`Nasdaq Traded`)
   stock_names <- gsub("\\$.*","", stock_names)
   stock_names <- as.data.frame(as.character(stock_names))
   
   stock_names$`as.character(stock_names)` <- as.character(stock_names$`as.character(stock_names)`)
   stock_names$`as.character(stock_names)` <- gsub("\\..*","", stock_names$`as.character(stock_names)`)
   stock_names <- unique(stock_names$`as.character(stock_names)`)
...
for ( i in data() ){
   ...
   tryCatch(
      ...
      bar_chart_rec(i), error = function(e){})
